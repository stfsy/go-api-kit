...
func GenericTest(t *testing.T, fn func(rw http.ResponseWriter, details ErrorDetails), status int, title string) {
	t.Parallel()
	assert := a.New(t)
	...
	for _, tc := range tests {
		t.Run(fmt.Sprintf("Sets status %d and marshalls %v", tc.status, tc.details), func(t *testing.T) {
			...
			assert.Nil(tc.details)
		})
	}
}

func GenericDetailsTest(t *testing.T, fn func(rw http.ResponseWriter, details ErrorDetails), status int, title string) {
	t.Parallel()
	assert := a.New(t)
	...
	var tests = []struct {
...

		{
			...
			status,
			ErrorDetails{
				"api_key": map[string]string{"message": "API key is missing"},
			},
		},
	}
...
func TestForbidden(t *testing.T) { GenericTest(t, SendForbidden, http.StatusForbidden, "Forbidden") }
func TestNotFound(t *testing.T)  { GenericTest(t, SendNotFound, http.StatusNotFound, "Not Found") }
...
func TestConflict(t *testing.T) { GenericTest(t, SendConflict, http.StatusConflict, "Conflict") }
func TestGone(t *testing.T)     { GenericTest(t, SendGone, http.StatusGone, "Gone") }
